...
| **prebuild 报 `No version matching "0.0.0-next-<n>" found for "@opencode-ai/cli-*" (but package exists)`** | **`BUN_CONFIG_REGISTRY` 指向 npmmirror,而国内镜像**不同步** opencode 的 `0.0.0-next-*` 预发布版(实测 npmjs 3192 版含 1035 个 next;npmmirror 85 版、next 为 0)。`CLI_VERSION`(`packages/desktop/scripts/utils.ts`)**每次上游同步都会变**,所以这坑会周期性复发** | **已修:该条 install 固定 `--registry`(默认 npmjs),见 `docs/features/desktop-build-blockers/`。若又见此错,先确认那行 `--registry` 还在;临时绕过用 `OPENCODE_CLI_REGISTRY=https://registry.npmjs.org bun run build`** |
| **验证打包时 electron zip 下载 600s 超时** | **绕过了 wrapper 直接调 `electron-builder`** —— wrapper `build-deskfox-electron.sh:212-217` 才配 `ELECTRON_MIRROR` / `ELECTRON_BUILDER_BINARIES_MIRROR` / 独立 `ELECTRON_CACHE`,裸调会走 GitHub + 代理 | **别绕过 wrapper**。它不只是"选 channel + 注版号",还封装了国内网络的三处规避;绕过得到的失败结论会误导排查方向(2026-08-18 就据此误报过一条"阻塞") |
