...
| (第三笔 follow-up) | **bug 5**(bug 4 修复引入的回归):视频生成完成、文件已落盘,但聊天区卡片永远停在"正在生成…" | 视频耗时几分钟,生成期间用户发 chat 建出 session → `adoptDraftInto` 把这张**运行中**的卡从 draft 移到新 session;而 `runCreation` 的 `patch` 用启动时**捕获的固定 scope** 找卡 → 卡已被移走 → patch 落空 → 永不更新为 done | `patch` 改按卡 **id 跨所有作用域查找**(卡可能被 adoption 移动),不再依赖固定 scope。+1 单测(可控 resolve 的 generate 模拟"生成中途被 adopt",验完成后仍 patch 到 done) |
| (第四笔 follow-up,`fix/media-creation-newsession-draft-reset`) | **bug 6**(bug 4 留的残留边界,user 追问后修):已停在首页时再点"新建会话"(home→home),旧 draft 卡不清 | 新建会话是同路由跳转(`/dir/session` 无 id),`params.id` 不变 → `session.tsx` 的 `on(params.id)` effect 不触发 → draft 不重置;多入口无单一 choke point,route-watch 测不到同路由 | store 加语义化 `resetDraft()`,在 4 个新建会话入口显式调:titlebar `+` 按钮 / 侧栏列表项 `<A>` / 侧栏工作区按钮 / `session.new` 命令(键盘 mod+shift+s + 命令面板)。clear draft 对进入已有会话无副作用,可无脑加。+1 单测 |

- **测试**:`utils/media-creation.test.ts` +4(`creationMediaSrc`:有本地走 localasset / 无本地回落远端 / 都无空串 / Win 反斜杠);`components/media-creation-store.test.ts` +5(作用域隔离 / resetScope 清空 / adoptDraftInto 过继 / 生成中途被 adopt 后仍能 patch 到 done / resetDraft 清首页 draft —— 注入 mock generate 不碰网络)。bug 1/2/6 是 View 层(组件挂载 / 按钮 wiring),e2e 基础设施未 setup 前由 user runtime 实测验证(同 window-resizable / right-click-stale-selection 等 view fix 先例)。
...
~~**残留边界**:首页 draft 在"已经停在首页时再点新建会话(home→home)"不会重置~~ —— **bug 6 已修**(第四笔 follow-up,4 个新建会话入口显式调 `resetDraft()`)。
