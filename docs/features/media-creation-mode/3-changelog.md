...
- **测试**:`utils/media-creation.test.ts` +4(`creationMediaSrc`:有本地走 localasset / 无本地回落远端 / 都无空串 / Win 反斜杠);`components/media-creation-store.test.ts` +4(作用域隔离 / resetScope 清空 / adoptDraftInto 过继 / 生成中途被 adopt 后仍能 patch 到 done —— 注入 mock generate 不碰网络)。bug 1/2 是 View 层(组件挂载位置),e2e 基础设施未 setup 前由 user runtime 实测验证(同 window-resizable / right-click-stale-selection 等 view fix 先例)。
