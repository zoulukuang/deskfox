...
## 发版前 review 收口(2026-08-19,mac prod 2026.11.1)

`/ship` 步骤 1 的 4-finder review(3 正确性角度 + 1 清理)在 `ship-mac-prod-2026.10.0..HEAD`
的 58 笔 / 产品代码 44 文件上抓出三条。**都不是崩溃级**(finder1 专审 REQ-084① 启动路径,
明确结论:三层 try-catch 全 fail-open,没有会让 app 起不来的问题),但都伤用户,
user 拍板**先修再发**,mac 因此走 2026.11.1(各平台独立号线)。

三条都先实测复现、再动手,每条配回归测试:

**① `\[…\]` / `$$` 块级劫持正文**(REQ-115 那批引入,不是本批;但在本次发版 diff 内)
块级 `start()` 无条件 `indexOf`,会在**任意位置**的 `\[` 处切开段落 —— 而 `\[` 在 markdown 里
最常见的用途是**转义字面方括号**。用 marked 18 实跑:`路径 C:\[temp\]` 渲染成「路径 C:」+ 公式块
「temp」;三个段落中间夹一对 `\[ \]` 会被整个吞进一个公式块(段落结构没了 + KaTeX 报错样式)。
修法:`start` 只认**行首**(≤3 空格缩进)的 `\[` / `$$`,块级正则**不跨空行**(空行是 markdown 块边界,
display math 内部不会有空行)。回归 +10 条,覆盖两条渲染路径 × 转义/跨段/shell `$$`/四种真公式写法。
⚠ **Win prod 2026.11.0 已带着这个问题发出去了**,Win 端需要跟一个补丁。

**② REQ-084① 隔离提示把用户指向错误目录**(本批自己引入)
`index.ts` 给 migrate 分支传的是 `dataHome`(**新**命名空间),而被判超前的 db 是**不迁**、
原件留在**旧** ns —— toast 文案偏偏是「原文件保留在:<dir>」,用户照着找必然找不到,
只会得出「数据被删了」。这条通知存在的唯一价值就是告诉用户数据在哪,等于反向失效。
修法:`ApplyResult` 新增 `quarantinedDir`(= `oldData`)。顺带把 toast 改 `persistent` ——
它一辈子只弹一次(主进程取走即清)、内容带着要复制的恢复路径,走默认 5 秒消失等于
「去倒杯水回来就再也找不到」。

**③ 基线「当前目录快照」→ append-only 并集,并更正一条错误的安全论证**
判 ahead 的安全前提是「用户跑过的任何一版内核写进 `migration` 表的 id 都仍在基线里」。
取快照的话,上游**改名/删除**一条我们已发布过的迁移,那条老 id 就从基线消失 →
老用户库里的它变成「基线外时间戳 id」→ 判超前 → **正在用的好库被改名挪走**。
上游确有改名先例(`20260530232709_lovely_romulus` → `20260511173437_session-metadata`),
只是尚未被任何已发布包带过,存量用户当前安全。
**同时更正 `db-schema-guard.ts` 的注释**:原写「legacy 名【不是】14 位时间戳形态,形态过滤能挡住」——
**这是错的**。core 把 `__drizzle_migrations.name` 原样搬进 `migration` 表,而上游迁移文件名一直就是
`YYYYMMDDHHMMSS_slug`(如 `20260127222353_familiar_lady_ursula`,至今在 `migration.gen.ts` 里)。
今天不误报只是因为那些老 id **恰好还在基线里**,不是被过滤挡住的 —— 真正的依靠是 append-only。

**④ 顺带补上一个空闸**:`packages/desktop` 此前**没有 test script**,那 169 条测试既不在 `turbo test`
也不在 pre-push 内,**写了从不自动跑** —— 其中就包括 ③ 的 drift 闸(「上游 sync 忘了重跑生成脚本」
的唯一防线)。pre-push 补 `(cd packages/desktop && bun test src/main/deskfox)`;按目录跑是因为
`draft-store.test.ts` 撞上游顶层 `import "node:sqlite"`(存量,见上文全批验收段)。

**本次未修、如实记录的**:
- share 页(`enterprise/routes/share`)对 `invalid`/`repeat` 分组无渲染分支 → 该行不显示。
  DeskFox 桌面端不走这条路径,仓内属回归,留给上游批处理。
- 行内 `$…$` 扩展在 marked 18 里跑在 codespan **之前**,`花了 $5,代码是 \`$total\` 变量`
  这类混排会吃掉行内代码。护栏对典型货币样例有效(实测无误伤),但顺序假设本身是错的。
- `TAB_COLLAPSE_DEFER_MS = 260` 短于两平台系统双击阈值(~500ms 且用户可调),260-500ms 的慢双击
  会误收面板。自动化点击间隔极短,天然绕开这个区间。

**flaky 登记(第二条,同族)**:`terminal-composer-focus.spec.ts:79` 在默认套件全量跑时偶发红
(焦点断言超时),单跑 `--repeat-each=3` **12/12 绿**,且它是**上游**测试文件(PR #38463)、
与本次改动零交集。与前面登记的 `review-line-comment` hover 同属「全套负载下的焦点/hover 类偶发」。
判据同上:再撞到就修时序或按 R5 显式移除,不靠 retry 掩盖。
