...
> 状态:**代码与自动化验证完成,待 🔒 真机验收 + user 拍板后合 main**。
...
- [ ] S3:真跑含 10+ 条 shell 的任务,默认折叠成一行;设置切回上游口径即时生效
- [ ] S7:真跑多次同文件编辑的任务,确认 `×N` 与文件名都在
- [ ] S4:真跑长任务全程盯列表;中途硬杀 / 断连重连后图标**不残留**
- [ ] S5:v2 点当前文件 tab 收起、再打开;双击开永久 tab 不被误伤;收起从右向左;经典布局不回退
- [ ] S9:让模型输出混排段(行内 `$` + `\[` 块 + 代码块 `$PATH` + 一个价格),肉眼确认
