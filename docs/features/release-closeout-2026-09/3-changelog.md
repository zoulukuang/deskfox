...
**未在 Win 上真机验、如实标注**:REQ-125(会话标题剥壳)、REQ-100(消息回吐 / 停止键)、REQ-128(工具行命中区)
需要真实 LLM 会话(本机 local 库无历史会话,模型走 user 的 Claude Code 订阅额度,未擅自消耗)。
三者均为**平台无关**的服务端字符串逻辑 / DOM 逻辑 / 纯 CSS,已由 Win 侧单测(opencode 424 / app 1129 / session-ui 121)
与 **Win e2e 142/142**(含 T11 工具行命中区、T13 引用卡片 tooltip)覆盖,mac 侧亦有真机 + user 自测记录。
