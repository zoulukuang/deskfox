...
**路径写法要与文件树拖入一致**。第一版直接插 `getPathForFile` 的原始路径,两个问题:
① Windows 上是 `C:\Users\…` 反斜杠原样;② 拖一个本来就在项目里的文件时给绝对路径,
而文件树拖同一文件给相对路径 —— 同一个文件两种引用写法。

改为复用既有 `absToRelPath`(导出为 `toMentionPath`),新增 W1–W5 五条测试专盯此事。
**这两条在 Mac 上就该测出来**(W3 与平台无关):验收用例列漏了「产物形态」这一维,
不是「Win 端才会暴露」。
