...
- [x] T17 选文字 → 加入聊天 → 不写正文发送 → **撤回按钮出现** → 点撤回 → 引用卡片回到输入框 → 补一句 → 重发成功
- [x] T18 撤回 → 点回滚坞「恢复」→ 输入框**没有残留重复卡片**(修正 2 的回归)
- [x] T19 正文 + 引用一起发 → 撤回 → 正文与卡片都回来,且重发后端无 `<chat selection>` 报错(REQ-119 联动)
