...
**pre-push 闸的去留已定(2026-09-19,user 拍板)**:**不进闸**,改挂 R9 合 main 前验收。
四条理由(风险位置 / 依赖面 / flake 零历史 / 冷态成本)与"什么时候才考虑进闸"的达标条件,
写在《自动化测试规范》R9 段的「上下文卡核心流程 e2e」条目。
同时按 hook 实况订正了 `CLAUDE.md` 与该规范里关于 `pre-push` 的两处漂移描述
(旧版写「仅 push 含 main 时」+「含 Phase 1 e2e」,实际无分支条件且闸内没有 e2e)。
