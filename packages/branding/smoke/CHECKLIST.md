...
| 7 | ~~文件关联~~ | **功能不存在**:CFBundleDocumentTypes 为空,基准版/历史/上游均无 → 已转需求池 ⏸ | — |
| 8 | 深链 `opencode://` | 正确路由 | `open "opencode://..."` |
| 9a | 拖文件进聊天输入框 | 变成附件进待发送区(含多文件、模型不支持图片时拦截) | 人工验收单 |
| 9b | 文件树内拖动移动文件 | 文件真移动 + 冲突有询问 | 人工验收单(**可自动化,待补**) |
| 10 | 首启引导(REQ-083) | 干净档案下建 New DeskFox + 自动打开该工作区(**用产品自带 `OPENCODE_TEST_ONBOARDING=1` 钩子**;老用户只建目录不自动打开) | `run_group1_native.py` |
| 11 | 更新器 UI | **本地版/预览版按设计禁用**(UPDATER_ENABLED),菜单项应置灰;完整对话框需 prod/beta 包 | `run_group1_native.py` + 人工(prod) |
| 12 | 崩溃/无响应恢复对话框 | **两层**:① 恢复对话框(重新启动/导出日志/退出,`windows.ts` wireWindowRecovery)② 连崩隔离(REQ-087,因 ① 默认重启而通常不触发) | `run_group1_native.py` |
