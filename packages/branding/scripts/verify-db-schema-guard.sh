...
#   默认 app:packages/desktop/dist-deskfox/mac-arm64/DeskFox 本地版.app
...
make_db() {
  ...
  {
    echo "CREATE TABLE migration (id TEXT PRIMARY KEY, time_completed INTEGER NOT NULL);"
    for id in "${BASE_IDS[@]}"; do echo "INSERT INTO migration VALUES ('$id', 1);"; done
    [ -n "$ahead" ] && echo "INSERT INTO migration VALUES ('$PROBE_ID', 1);"
    # 一张有用户数据的表,用来验证"数据是否真的被带过去/留下"
    echo "CREATE TABLE probe_marker (tag TEXT);"
    echo "INSERT INTO probe_marker VALUES ('QA-PROBE-DATA');"
  } | sqlite3 "$path"
}

has_probe_row() { sqlite3 "$1" "SELECT count(*) FROM migration WHERE id='$PROBE_ID';" 2>/dev/null || echo "ERR"; }
...
OLD_DB_MD5="$(md5 -q "$T4_OLD/$DB_NAME")"
...
if [ "$(md5 -q "$T4_OLD/$DB_NAME")" = "$OLD_DB_MD5" ]; then
...
if [ -n "$QUARANTINED" ]; then
  ...
  if [ -n "$MAIN_Q" ] && [ "$(md5 -q "$MAIN_Q")" = "$T5_DB_MD5" ]; then
...
if [ -f "$T6_NS/$DB_NAME" ]; then
  ok "正常库仍在原位"
  if sqlite3 "$T6_NS/$DB_NAME" "SELECT tag FROM probe_marker;" 2>/dev/null | grep -q "QA-PROBE-DATA"; then
    ok "库内用户数据完好(未被清空/重建)"
  else
