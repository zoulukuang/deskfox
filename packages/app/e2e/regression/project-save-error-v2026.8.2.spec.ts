...
//  选择器纪律(2026-06-18):对话框/toast 用 data-component;菜单/按钮用 role;locale 已钉 en-US,
//  不用裸 getByText 文案。
import { expect, test, type Page } from "@playwright/test"
import { mockOpenCodeServer } from "../utils/mock-server"
import { expectAppVisible } from "../utils/waits"
import { makeProject, makeProvider } from "../utils/fixtures"
...
test.describe("regression: edit-project save failure shows toast (REQ-064 v2026.8.2)", () => {
  test("failing project.update surfaces an error toast and keeps the dialog open", async ({ page }) => {
    ...
    // 右键左侧项目头像 → 上下文菜单"编辑"(page.mouse 原始右键,与真机一致)
