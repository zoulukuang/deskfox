...
// FORK: REQ-072 — V1(legacy sidecar)路径必须显式传 scope=project,后端才按项目身份列会话
// (改名/挪位/复制跟随);上游重构 loadRootSessions 拆 v2/v1 后,scope 注入落在 loadRootSessionsV1。
describe("loadRootSessionsV1 — scope=project (REQ-072)", () => {
  test("limited roots query carries scope=project", async () => {
  ...
  test("fallback full roots query also carries scope=project", async () => {
    const calls: Array<Record<string, unknown>> = []
    const result = await loadRootSessionsV1({
      client: {
        session: {
