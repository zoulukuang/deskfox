...
export const { use: usePermission, provider: PermissionProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    return {
      ...
      // FORK: REQ-078 canResolve 委托当前 server 状态 [feat: permission-filter-concurrency] 2026-08-02
      canResolve(permission: PermissionRequest, directory?: string) {
        return selected().canResolve(permission, directory)
      },
...
function createServerPermissionState(input: { sdk: ServerSDK; sync: ServerSync }) {
  ...
  // FORK-BEGIN: REQ-078 方案D「谁触发谁展示」共享过滤层(per-server 化移植 2026-08-11)
  // [feat: permission-filter-concurrency] 2026-08-02
  // 按 directory 维护「本 instance 可 resolve 的权限 id 集」;无候选不发请求;失败/加载中 fail-open。
  ...
  const resolvableCache = createResolvableCache(async (directory) => {
    const r = await input.sdk.client.permission.list({ directory })
  ...
  const api = {
    ...
    // FORK: REQ-078 [feat: permission-filter-concurrency]
    canResolve(permission: PermissionRequest, directory?: string) {
      if (meta.disposed) return true
      return canResolve(permission, directory)
    },
    toggleAutoAccept(sessionID: string, directory: string) {
      if (meta.disposed) return
