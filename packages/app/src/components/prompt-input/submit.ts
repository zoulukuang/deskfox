...
//   用户再发一次就是重复的一轮。
//   概率评估:正常 admit 是毫秒级返回,20 秒静默**绝大多数**意味着根本没走到 admit,
//   所以这是窄边角而非常态;且两害相权,"消息还在你手里"比"消息凭空消失"轻得多,回吐语义保留。
//   本次处置 = 把断言改成不把话说死(toast 文案从「这条没发出去」改为「可能没发出去」+
//   明确提示"重发前先看一眼对话里是否已经有了"),**不改行为**。
//   根治要靠幂等核对(messageID 是前端生成的,具备去重条件):恢复连接后按 ID 查一次,
//   已落地就撤掉输入框里那份并告知用户。已入需求池,见「消息未送达提示可能误报-需幂等核对」。
//
