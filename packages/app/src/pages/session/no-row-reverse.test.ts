...
test("经典布局镜像不得使用 flex-row-reverse(一律改用 order)", () => {
  ...
  // FORK 2026-09-17 [feat: release-closeout-2026-09]:显式放宽超时到 60s。
  // [bug-repro: Win 侧回验时本条在全量 `bun run test:unit` 里超时假红(5633ms / 默认 5s),
  //  单跑却只要 224ms、复跑全量也 0 fail —— 那一轮整套耗时从 4.5s 膨胀到 21.5s,是机器负载的指纹。
  //  实测本用例真实成本仅 ~80ms(walk 602 个文件 31ms + stripComments 48ms),与文件系统无关,
  //  所以不是「Win 上慢」而是「默认 5s 对一条同步扫全树的用例余量太薄」:机器一忙就假红。
  //  守卫类测试假红的代价特别高 —— 它会让人怀疑守的那件事,而不是怀疑时钟。给足余量,不改逻辑。]
}, 60_000)
